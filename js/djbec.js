...
function bi(s, base) { return new BigInteger(s, base); }
...
var xff = bi('255');
var b = bi('256');
var q = two.pow(bi('255')).subtract(bi('19'));
...
var i = two.modPow(q.subtract(one).divide(bi('4')), q);

function xrecover(y) {
  var ysquared = y.times(y);
  var xx = ysquared.subtract(one).times(inv(one.add(d.times(ysquared))));
  var x = xx.modPow(q.add(bi('3')).divide(bi('8')), q);
  if (!(x.times(x).subtract(xx).mod(q).equals(zero))) {
    x = x.times(i).mod(q);
  }
...
function decodeint(a) {
  var n = bi('0');
  for (var i = 0; i<32; i++) {
    n=n.shiftLeft(8).add(bi(''+a[31-i]));  // Blech!!!
  }
  return n;
}
...
function inthash(s) {
  return bytes2bi(bi2bytes(hex2bi(sha512(s)), 64).reverse())
}

function stringhash(s) {
  return bytes2string(bi2bytes(hex2bi(sha512(s)), 64).reverse());
}
